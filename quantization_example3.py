...
# Create a copy of the model for static quantization
static_quantized_model = torch.quantization.quantize_static(
    non_quantized_model, qconfig=torch.quantization.default_qconfig, dtype=torch.qint8
)
print(static_quantized_model)

start_time = time.time()
test_loss, accuracy = test(static_quantized_model, test_loader)
static_quantized_model_time = time.time() - start_time

print('\nStatic Quantized model:')
print('Test set: Average loss: {:.4f}, Accuracy: {:.2f}%'.format(test_loss, accuracy))
print('Time taken for inference: {:.2f} seconds'.format(static_quantized_model_time))
