...
DATA_TEXT_FILE = 'data_onnx_desktop.txt' # save the average database and query times to this file
...
bruh = db_features_onnx.detach().cpu().numpy()
...
similarity_matrix = cosine_similarity(db_features_onnx.detach().numpy(), q_features_onnx.detach().numpy())
...
with open(DATA_TEXT_FILE, 'w') as f:
    ...
    f.write(f"Database Average Time: {db_time_onnx}\n")
